...
remove_redundancy <- function(file, sheet) {
  ...
  # excel sheet
    y <- fread(file) %>%
      ...
      filter(padj < 0.05)
    # reorder cols
    y <- y[, c(ncol(y), 1, 8:10, 18:20, 12:17, 11, 2:7)]
    ...
    names(y) <- c("Contrast", "MeDIP-Seq peak", "Chromosome",
    ...
    upd_nam <- gsub("hPSCs_|NCRM5\\.", "", sheet)
    addWorksheet(xlwb, paste(gsub("_", "_vs_", upd_nam), sep = ""))
    writeData(xlwb, 
              sheet = gsub("_", "_vs_", upd_nam), 
              x = as.data.frame(y))
}