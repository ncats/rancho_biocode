...
lapply(1:length(gsea_all), function(x) {
  ...
  if (nrow(y) == 0) {
    ...
    y <- y[c(1:10),]
    CairoPDF(file = paste("./adj_data/plots/gsea/gsea_", gsub("_", "_vs_", all_nam[[x]]), "_dotplot.pdf", sep = ""),
             width = 15, height = 12,
             family = "Noto Sans Cond")
    print(ggplot(y, aes(x = reorder(description, Count), y = Count, fill = p.adjust)) +
            geom_point(aes(size = GeneRatio), shape = 21) + coord_flip() +
            ...
            theme_minimal() + 
            ggtitle(paste("Significant GSEA for ", gsub("_", " vs ", all_nam[[x]]), " from GO/KEGG gene sets", sep = "")) +
