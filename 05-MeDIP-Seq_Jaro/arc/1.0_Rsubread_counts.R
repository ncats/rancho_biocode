...
# load fastq files
fastq.files <- list.files(path = "./FASTQ_MeDIP3", pattern = ".fastq.gz$", full.names = TRUE)
fastq.files <- fastq.files[-1]

nam <- gsub(".*\\/|\\.fastq.gz", "", fastq.files)
out <- paste("./BAM/", nam, "_subread_results.bam", sep = "")

# load dir of bam files
bam <- list.files("./BAM_MeDIP3/", pattern = ".bam$", full.names = T)

# create a saf file based on mergedregions
# identified by active motif
mr <- readWorkbook("./data/4441NIH_J98i-MeDIP_mergedregs.xlsx")
saf_mr <- mr[, c(1:4)] %>%
    mutate(Strand = "*")
